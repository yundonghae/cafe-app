/* ============================================
   🌊 관리자 - 메뉴 추가 (create.html)
   폼 검증 → CafeData.createMenu() → 목록으로 이동
   ============================================ */

(function () {
  const { $, formatPrice, escapeHtml } = window.CafeUtils;
  const { getCategories, getCategoryById, createMenu } = window.CafeData;
  ...
  form.addEventListener("submit", (e) => {
    ...
    const created = createMenu(values);

    // 목록 페이지에서 띄울 안내 메시지를 남기고 이동한다
    sessionStorage.setItem(
      "cafe.flash",
      `'${created.name}' 메뉴를 새로 띄웠습니다.`
    );
